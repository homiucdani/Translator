...
fun LanguageDropDownMenu(
    ...
    Box(modifier = modifier) {
        DropdownMenu(
            ...
            UiLanguage.allLanguages.forEach { item ->
        ...
        // this is the anchor, dropdown will be visible only when its open, otherwise this row it is
        Row(modifier = Modifier
            .clickable { onClick() }
            .padding(16.dp),
            verticalAlignment = Alignment.CenterVertically
        ) {
            // bug with vector using image from compose
            AsyncImage(
